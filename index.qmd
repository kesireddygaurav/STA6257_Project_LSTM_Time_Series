...
final$converted = final$predictions * ( max(mean_temp$meantemp) - min(mean_temp$meantemp) ) + min(mean_temp$meantemp)
final$original = test$meantemp 
final$date = test$date
#final <- invert_scaling(predictions, scaler)
...
ggplot(data = final) +
  ...
  geom_line( aes(x = date, y = converted), color = "red") +
...
final$diff=(final$original-final$converted)**2

MSE=sum(final$diff)/length(final$diff)
...
ts_data <- ts(mean_temp$transformed)

# Load the 'TSLSTM' package
library(TSLSTM)

# Use the ts.lstm function with the ts object
result <- ts.lstm(
  ts = ts_data,
  tsLag = 1,
  xreg = NULL,
